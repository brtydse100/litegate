...
async function mockApi(
  ...
  await page.route(/^https?:\/\/[^/]+\/api\//, async (route) => {
    ...
    else if (url.pathname === "/api/v1/keys/identifiers")
      body = { keys: ["key-1", "key-2"], total: 2 };
...
test("@smoke administrator navigation has stable routes and global selection", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("LiteGate is ready to serve requests."),
  ).toBeVisible();

  const usersRequest = page.waitForRequest(
    (request) => new URL(request.url()).pathname === "/api/v1/users",
  );
  await page.getByRole("link", { name: "Local users" }).click();
  await usersRequest;
  await expect(
    page.getByRole("heading", { name: "Local user access" }),
  ).toBeVisible();
});

test("@smoke normal users cannot open administrator routes", async ({
  page,
}) => {
...
test("administrator can create, edit, move a member, and delete a team", async ({
  page,
}) => {
  const team = (
    team_id: string,
  ...
  let teams = [
    team("team-source", "Source", [
      { user_id: "alice", user_email: "alice@example.com", role: "user" },
    ]),
  ...
  await page.route(/^https?:\/\/[^/]+\/api\//, async (route) => {
    ...
    if (url.pathname === "/api/auth/me")
    ...
    else if (url.pathname === "/api/v1/teams" && method === "GET")
      body = {
        teams,
        total: teams.length,
        page: 1,
      ...
      const created = {
        ...team(payload.team_id || "generated-team", payload.team_alias),
        ...payload,
      };
      teams = [...teams, created];
      body = created;
      ...
      teams = teams.map((item) =>
        item.team_id === id ? { ...item, ...payload } : item,
      );
      body = teams.find((item) => item.team_id === id);
    } else if (
      url.pathname === "/api/v1/teams/team-source/members/move" &&
      method === "POST"
    ) {
      movePayload = route.request().postDataJSON();
      teams = teams.map((item) =>
        item.team_id === "team-source"
          ? { ...item, members_count: 0, members_with_roles: [] }
          : item,
      );
      body = {
        moved: true,
  ...
  await members
    .getByRole("combobox", { name: /Destination team/ })
    .selectOption("team-destination");
  ...
  await deleteDialog
    .getByRole("button", { name: "Delete team and keys" })
    .click();
  await expect(page.getByRole("heading", { name: "Research" })).toHaveCount(0);
});

test("administrator can manage a local account without leaving the users page", async ({
  page,
}) => {
  let users: Array<{
    username: string;
  ...
  await page.route(/^https?:\/\/[^/]+\/api\//, async (route) => {
    ...
    if (url.pathname === "/api/auth/me")
      ...
      const created = {
        ...payload,
        user_id: `local:${payload.username}`,
        active: true,
      ...
      url.pathname === "/api/v1/users/contractor" &&
      method === "PATCH"
    ) {
      const payload = route.request().postDataJSON();
  ...
  const reset = page.getByRole("dialog", {
    name: "Reset contractor's password",
  });
